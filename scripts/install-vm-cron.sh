...
echo ""
echo "Test HTTP crons (calls web container directly — avoids nginx 301 on http://127.0.0.1):"
echo "  bash $CALL appointment-reminders"
echo "  bash $CALL kai-weekly"
echo "Backup test: bash $BACKUP"
echo ""
echo "If docker mode fails, use: CRON_MODE=http CRON_BASE_URL=https://YOUR_DOMAIN CRON_INSECURE=0 bash $CALL kai-weekly"
