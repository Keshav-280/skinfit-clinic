...
function OrbitNode({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className="absolute flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full bg-white/90 shadow-[0_10px_24px_-10px_rgba(44,62,107,0.45)] transition"
      style={{
        left: x,
        top: y,
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-[440px] px-4 pb-12 pt-6">
        <p className="text-center text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#2C3E6B]/55">
        ...
        <div className="relative mx-auto mt-4 h-[340px] w-[340px]">
          <div className="pointer-events-none absolute inset-[18px] rounded-full border border-[#2C3E6B]/12" />
          <div className="absolute left-1/2 top-1/2 h-[188px] w-[188px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full shadow-[0_20px_50px_-18px_rgba(44,62,107,0.45)] ring-4 ring-white/80">
            <ConceptFace
              url={data.faceUrl}
        ...
        {active ? (
          <div className="mx-auto mt-2 max-w-[22rem] rounded-[22px] bg-white/80 px-4 py-3.5 text-center shadow-[0_16px_40px_-22px_rgba(44,62,107,0.4)] backdrop-blur-md">
            <p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#2C3E6B]/55">
        ...
        <div className="mt-6 flex flex-col gap-2">
          {data.actions.map((text, i) => (
            <div
              key={i}
              className="flex items-center gap-3 rounded-full bg-white/80 px-3.5 py-3 shadow-[0_10px_28px_-18px_rgba(44,62,107,0.4)]"
            >
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#2C3E6B] text-[12px] font-bold text-white">
                {i + 1}
              </span>
        ...
        <Link
          ...
          className="mt-6 block rounded-full bg-[#2C3E6B] py-3.5 text-center text-[14px] font-semibold text-white shadow-[0_16px_32px_-12px_rgba(44,62,107,0.55)]"
        >
          Book a Medixora scan
        </Link>
        <Link
          href="/dashboard/chat?assistant=doctor"
          className="mt-2 block py-2 text-center text-[13px] font-semibold text-[#2C3E6B]/70"
        >
