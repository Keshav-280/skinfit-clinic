...
export async function GET(req: Request) {
  ...
  if (waterRecords.length === 0) {
    return NextResponse.json({
      insight: "Start logging your water intake daily to get personalized hydration insights!",
  ...
  if (!client) {
    const avgGlasses = Math.round(waterRecords.reduce((s, r) => s + r.glasses, 0) / waterRecords.length);
  ...
  const logSummary = waterRecords
    .map((r) => `${r.date}: ${r.ml}ml (${r.glasses} glasses), sleep=${r.sleep ?? "—"}h, stress=${r.stress ?? "—"}, sun=${r.sun ?? "—"}`)
    .join("\n");
...
Respond in JSON: {"insight": "...", "tip": "..."}`;
  ...
  try {
    ...
    return NextResponse.json({
      insight: parsed.insight ?? "Keep tracking your hydration for personalized insights!",
      tip: parsed.tip ?? "Aim for 8 glasses of water daily.",
    });
  } catch {
    const avgGlasses = Math.round(waterRecords.reduce((s, r) => s + r.glasses, 0) / waterRecords.length);
    return NextResponse.json({
      insight: avgGlasses >= 8
