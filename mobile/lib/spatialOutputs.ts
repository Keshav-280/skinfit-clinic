/** Spatial head outputs (wrinkle seg map + acne 16×16 grid). Mirrors web `src/lib/spatialOutputs`. */
...
export function parseSpatialOutputsFromAnalysis(
  analysis: unknown
): ScanSpatialOutputs | undefined {
  if (!analysis || typeof analysis !== "object") return undefined;
  const root = (analysis as Record<string, unknown>).spatialOutputs;
...
export function formatWrinkleHeadDetail(
  spatial?: ScanSpatialOutputs | null
): string | null {
  if (!spatial?.wrinkles) return null;
  const w = spatial.wrinkles;
  return `Seg map · Cls ${w.cls_severity_1_5.toFixed(1)} · Seg ${w.seg_severity_1_5.toFixed(1)} · Combined ${w.combined_severity_1_5.toFixed(1)}`;
}

export function formatAcneHeadDetail(
  spatial?: ScanSpatialOutputs | null
): string | null {
  if (!spatial?.acne) return null;
  const a = spatial.acne;
  return `16×16 grid · Global ${a.global_severity_1_5.toFixed(1)} · Patch mean ${a.patch_mean.toFixed(3)}`;
}
