...
const MOTIVATIONS = [
  ...
  "Stay the course, see results.",
];

type HomeData = {
  amItems: string[];
  pmItems: string[];
  todayLog: { routineAmSteps?: boolean[] | null } | null;
...
export default function MorningRoutineScreen() {
  ...
  const total = data?.amItems.length ?? 0;
  ...
  function markAll() {
    if (!data) return;
    const next = data.amItems.map(() => true);
    setSteps(next);
    void saveSteps(next);
  }
  ...
  if (!data || !data.routinePlanReady || data.amItems.length === 0) {
    return (
      <View style={[styles.center, { paddingTop: insets.top }]}>
        <Pressable onPress={() => router.back()} style={styles.backBtn} hitSlop={12}>
          <Ionicons name="arrow-back" size={22} color={NAVY} />
        </Pressable>
        <Text style={styles.empty}>Your morning routine hasn't been set up yet. Your clinic will configure it soon.</Text>
      </View>
    );
  }
      ...
      <View style={styles.reminderCard}>
        <View style={styles.reminderLeft}>
          <View style={styles.reminderIcon}>
            <Ionicons name="sunny" size={22} color="#FDE68A" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.reminderKicker}>Daily Reminder</Text>
            <Text style={styles.reminderTitle}>{motivation}</Text>
            <Text style={styles.reminderSub}>Follow your routine every morning.</Text>
          </View>
        </View>
      ...
      <View style={styles.whyCard}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.whySub}>
            This routine protects your skin from UV damage and keeps it hydrated all day.
          </Text>
        </View>
      </View>
    </ScrollView>
